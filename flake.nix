{
  ...
  outputs = {
      ...
      modules = [
        (addPackageKeys {
          ...
          packages = {
            ...
            cardano-crypto-wallet.doHaddock = false;
          };
        })
      ];
    };
    ...
    mkCompilerPackageTreeWith = f: pkg-versions: system:
      lib.genAttrs (compilersForSystem system) (compiler: let
        filtered-pkgs-versions =
          ...
          (name: versions: let
            predicate =
              ...
              exceptions;
          in
            builtins.filter predicate versions)
          pkg-versions;
      in
    ...
    # -> Map String a
    flattenTree = lib.concatMapAttrs (compiler:
      lib.concatMapAttrs (name:
        lib.concatMapAttrs (
          version: value: {"${compiler}/${name}/${version}" = value;}
        )));
  in
